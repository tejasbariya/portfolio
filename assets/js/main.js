(async function () {
  ...
  // The HTML already ships with real, static fallback content for SEO/crawlability —
  // if this fetch fails (offline, path issue, CMS down) we simply keep that content
  ...
  try {
    ...
    console.warn('Could not load data/data.json — keeping static fallback content.', err);
  }

  // --- START DYNAMIC CONTENT INJECTION ---
  if (DATA && document.getElementById('site-title')) {
    ...
    // Tech Stack — only re-render if the CMS actually returned a stack; otherwise keep
    // the static tags that already shipped in the HTML.
    ...
    // Info Cards
    if ((DATA.focus && DATA.focus.length) || (DATA.learning && DATA.learning.length)) {
    ...
    // Projects Grid — re-render from fresh CMS data (replaces static fallback cards).
        ...
        card.innerHTML = `
          <div class="pc-header">
            ...
            <div class="pc-links-row">
              ${p.github ? `<a href="${p.github}" target="_blank" rel="noopener" class="pc-icon" title="${p.title || 'Project'} on GitHub">${GH_ICON}</a>` : ''}
              ${p.live   ? `<a href="${p.live}"   target="_blank" rel="noopener" class="pc-icon" title="${p.title || 'Project'} live demo">${LIVE_ICON}</a>` : ''}
            </div>
          </div>
  ...
  // Terminal Widget (uses DATA if available, otherwise leaves the widget empty gracefully)
  const termBody = document.getElementById('termBody');
  ((DATA && DATA.terminal) || []).forEach(row => {
  ...
  // Filter Bar Logic — build category list from whatever project cards are currently in the DOM
  // (works whether they came from DATA or from the static fallback markup).
  const filterBar = document.getElementById('filterBar');
  const projCount = document.getElementById('projCount');
  const cats = ['all', ...new Set(Array.from(document.querySelectorAll('.proj-card')).map(c => c.dataset.cat).filter(Boolean))];
  ...
  projCount.textContent = document.querySelectorAll('.proj-card').length;
  ...
  contactForm.addEventListener('submit', function (e) {
    ...
    fetch((DATA && DATA.formEndpoint) || 'https://formspree.io/f/xaqpalbv', {
      method: 'POST',
    ...
    .catch(err => {
      console.error(err);
      setStatus('Failed — email me directly at ' + ((DATA && DATA.email) || 'tejasbariya@gmail.com'), true);
    })
